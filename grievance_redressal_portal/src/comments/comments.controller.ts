...
export class CommentsController {
    ...
    @Patch('update_comment_content/:comment_id/:parent_comment_id')
    updateCommentContent(@Param('comment_id', ParseIntPipe) comment_id: number, @Param('parent_comment_id', ParseIntPipe) parent_comment_id: number, updateCommentContentDto: UpdateCommentContentDto) {
        console.log("In updateCommentContent");
        return this.commentsService.updateCommentContent(comment_id, parent_comment_id, updateCommentContentDto);
    }

    @Patch('soft_delete_reply/:comment_id/:parent_comment_id')
    softDeleteReply(@Param('comment_id', ParseIntPipe) comment_id: number, @Param('parent_comment_id', ParseIntPipe) parent_comment_id: number, softDeleteReplyDto: SoftDeleteReplyDto) {
        console.log("In softDeleteReply");
        return this.commentsService.softDeleteReply(comment_id, parent_comment_id, softDeleteReplyDto);
    }

    @Delete('hard_delete_reply/:comment_id/:parent_comment_id')
    hardDeleteReply(@Param('comment_id', ParseIntPipe) comment_id: number,  @Param('parent_comment_id', ParseIntPipe) parent_comment_id: number) {
        console.log("In hardDeleteReply");
        return this.commentsService.hardDeleteReply(comment_id, parent_comment_id);
    }
}